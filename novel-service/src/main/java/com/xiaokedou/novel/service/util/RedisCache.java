...
public class RedisCache {
	...
	private Object deserilize(byte[] object, RedisAnontation anontation) {
		if (anontation.serialType() == RedisAnontation.SerialType.LIST) {
			// 查询到了，反序列化
			return SerializeUtil
					.deserializationList(object, anontation.clazz());
		} else if (anontation.serialType() == RedisAnontation.SerialType.OBJ) {
			// 查询到了，反序列化
			return SerializeUtil.deserializationObject(object,
					anontation.clazz());
		} else if (anontation.serialType() == RedisAnontation.SerialType.SET) {
			// 查询到了，反序列化
			return SerializeUtil.deserializationSet(object, anontation.clazz());
		} else if (anontation.serialType() == RedisAnontation.SerialType.MAP) {
			// 查询到了，反序列化
