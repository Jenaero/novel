                    ...
                    while (iterator.hasNext()) {
                        ...
                        for (int i = 0; i < novels.size(); i++) {
                            ...
                            LambdaQueryWrapper <Novel> novelLambdaQueryWrapper = new LambdaQueryWrapper <Novel>()
                                    ...
                                    .eq(Novel::getStatus, novel.getStatus();
                            Integer novelCount = novelMapper.selectCount(novelLambdaQueryWrapper)
                            );
                            if (novelCount <= 0) {
                                logger.warn("author=" + novel.getAuthor() + ",name=<" + novel.getName() +
                                        ">,status="+NovelStatusEnum.getLabelByKey(novel.getStatus())+",不存在，不用更新！");
                                continue;
                            }
                            if (novelCount > 1){
                                logger.warn("author=" + novel.getAuthor() + ",name=<" + novel.getName() +
                                        ">,status="+NovelStatusEnum.getLabelByKey(novel.getStatus())+",有"+novelCount"本,数据异常,不更新！");
                                continue;
                            }
                            //todo 更新列表处理逻辑再想想
                            /** 1、最后更新时间
